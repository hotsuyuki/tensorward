...
class SumTo : public core::Function {
 public:
  ...
  const std::vector<xt::xarray<float>> Backward(const std::vector<xt::xarray<float>>& dL_dys) override {
    ...
    const xt::xarray<float> dL_dx = xt::broadcast(dL_dys[0], x_shape);
...
const core::TensorSharedPtr sum_to(const core::TensorSharedPtr input_tensor_ptr,
  ...
  const core::FunctionSharedPtr transpose_function_ptr = std::make_shared<SumTo>(output_shape);
