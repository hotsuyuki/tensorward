 ...
 protected:
  AddTest()
      : input_data0_(xt::random::rand<float>({kHight, kWidth})),
        ...
        add_function_ptr_(std::make_shared<Add>()) {}

  const xt::xarray<float> input_data0_;
  const xt::xarray<float> input_data1_;
  const xt::xarray<float> expected_output_data_;
  const FunctionSharedPtr add_function_ptr_;
};

TEST_F(AddTest, ForwardTest) {
  const std::vector<xt::xarray<float>> actual_input_datas({input_data0_, input_data1_});
  const std::vector<xt::xarray<float>> actual_output_datas = add_function_ptr_->Forward(actual_input_datas);
...
TEST_F(AddTest, AnalyticalBackwardTest) {
  ...
  const std::vector<TensorSharedPtr> actual_output_tensors = add_function_ptr_->Call(actual_input_tensors);
  ...
  const std::vector<xt::xarray<float>> actual_input_grads = add_function_ptr_->Backward(actual_output_grads);
...
TEST_F(AddTest, NumericalBackwardTest) {
  // NOTE: Need to use `Call()` instead of `Forward()` in order to create the computational graph for `Backward()`.
  const std::vector<TensorSharedPtr> actual_input_tensors(
      {AsTensorSharedPtr(input_data0_), AsTensorSharedPtr(input_data1_)});
  const std::vector<TensorSharedPtr> actual_output_tensors = add_function_ptr_->Call(actual_input_tensors);
  ...
  const std::vector<xt::xarray<float>> expected_input_grads =
      util::NumericalGradient(add_function_ptr_, {input_data0_, input_data1_}, kEpsilon);
  ASSERT_EQ(expected_input_grads.size(), 2);
...
TEST_F(AddTest, CallWrapperTest) {
  ...
  // `add()` is a `Function::Call()` wrapper.
  const TensorSharedPtr output_tensor_ptr = add(input_tensor_ptr0, input_tensor_ptr1);
...
TEST_F(AddTest, OverloadedOperatorTest) {
  const TensorSharedPtr input_tensor_ptr0 = AsTensorSharedPtr(input_data0_);
  const TensorSharedPtr input_tensor_ptr1 = AsTensorSharedPtr(input_data1_);
