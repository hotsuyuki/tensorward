...
TEST_F(AddTest, ForwardTest) {
  const std::vector<xt::xarray<float>> actual_output_datas = add_function_ptr_->Forward({input_data0_, input_data1_});
...
TEST_F(AddTest, AnalyticalBackwardTest) {
  ...
  const std::vector<TensorSharedPtr> actual_output_tensors =
      add_function_ptr_->Call({AsTensorSharedPtr(input_data0_), AsTensorSharedPtr(input_data1_)});
