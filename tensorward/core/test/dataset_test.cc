...
class DatasetTest : public ::testing::Test {
 protected:
  DatasetTest()
      : transform_to_half_lambda_([](const xt::xarray<float>& input_data) { return input_data / 2.0; }),
        transform_to_onehot_lambda_([](const xt::xarray<float>& input_data) {
        ...
        data_transform_lambdas_({transform_to_half_lambda_}),
        label_transform_lambdas_({transform_to_onehot_lambda_}),
        spiral_dataset_(kIsTrainingMode, data_transform_lambdas_, label_transform_lambdas_) {}

  const TransformLambda transform_to_half_lambda_;
  const TransformLambda transform_to_onehot_lambda_;
...
TEST_F(DatasetTest, AtTest) {
  for (std::size_t i = 0; i < spiral_dataset_.size(); ++ i) {
    ...
    // Prepares the expected i-th data, and applies the "transform-to-half" transform to it.
    xt::xarray<float> expected_ith_data = ith_data;
    expected_ith_data = expected_ith_data / 2.0;

    // Prepares the expected i-th label, and applies the "transform-to-onehot" transform to it.
